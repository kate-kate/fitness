...
<?php 
	if (!post_password_required()) {
		if ($cmsms_post_video_type == 'selfhosted' && !empty($cmsms_post_video_links) && sizeof($cmsms_post_video_links) > 0) {
			...
			echo '<div class="cmsms_video_wrap">' . 
				do_shortcode($wp_embed->run_shortcode('[embed width="' . esc_attr($video_size['full-thumb']['width']) . '" height="' . esc_attr($video_size['full-thumb']['height']) . '"]' . $cmsms_post_video_link . '[/embed]')) . 
			'</div>';
