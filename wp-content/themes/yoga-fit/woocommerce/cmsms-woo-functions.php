...
function cmsms_woocommerce_add_to_cart_button() {
	...
	if (
		...
		echo '<a href="' . esc_url($product->add_to_cart_url()) . '" data-product_id="' . esc_attr($product->id) . '" data-product_sku="' . esc_attr($product->get_sku()) . '" class="add_to_cart_button cmsms_add_to_cart_button product_type_simple button">' . esc_html__('Add to Cart', 'yoga-fit') . '</a>';
	}
}
